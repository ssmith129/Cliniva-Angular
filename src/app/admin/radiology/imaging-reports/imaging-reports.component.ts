...
import { ImagingReportsFormComponent } from './dialogs/form-dialog/form-dialog.component';
import { ImagingReportsDeleteComponent } from './dialogs/delete/delete.component';
...
@Component({
  ...
  styleUrls: ['./imaging-reports.component.scss'],
  standalone: true,
  imports: [BreadcrumbComponent, MasterTableComponent],
})
export class ImagingReportsComponent implements OnInit, OnDestroy {
  ...
  columnDefinitions: ColumnDefinition[] = [
    ...
    { def: 'radiologistName', label: 'Radiologist', type: 'text', visible: true },
  ...
  dataSource = new MatTableDataSource<ImagingReport>([]);
  isLoading = true;
  ...
  handleEdit(row: ImagingReport) {
    this.openDialog('edit', row);
  }

  handleDelete(row: ImagingReport) {
    const dialogRef = this.dialog.open(ImagingReportsDeleteComponent, {
      data: row,
    });
  ...
  handleBulkDelete(selectedRows: ImagingReport[]) {
  ...
  openDialog(action: 'add' | 'edit', data?: ImagingReport) {
    const dialogRef = this.dialog.open(ImagingReportsFormComponent, {
      width: '60vw',
      maxWidth: '100vw',
